...
object KernelDensityHelper {
  def stampNeigh(raster: RasterExtent, v: Int, x: Int, y: Int,
                                    dx: Int, dy: Int, kernel: Array[Int], data: MutableRasterData):Unit = {
    var k = 0
    val nr = raster.rows
    val nc = raster.cols
    var c:Int = x - dx / 2
...
extends Op3(outputRasterExtent, kernel, points) ({
  (re, kernel, pts) => {
    val data:MutableRasterData = new IntArrayRasterData(Array.ofDim(re.rows * re.cols), re.cols, re.rows)
    val k = kernel.data
    ...
    while(ptIdx < ptLen) {
      val pt = pts(ptIdx)
      val v = if (pt.value == 0) 1 else pt.value
      ...
      if ((px > -w2 && px < cols + w2) && (py > -w2 && py < rows + w2)) {
        KernelDensityHelper.stampNeigh(re, v, px, py, w, w, k, data)
        }
  
      ptIdx += 1
      }

    Result(new Raster(data, re))
    }
})
